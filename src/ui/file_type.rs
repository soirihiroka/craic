...
pub(in crate::ui) const MIME_FOLDER: &str = "inode/directory";
pub(in crate::ui) const MIME_MARKDOWN: &str = "text/markdown";
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::ui) struct FileType {
    pub(in crate::ui) mime: &'static str,
    pub(in crate::ui) icon_name: &'static str,
    pub(in crate::ui) display_kind: &'static str,
}

pub(in crate::ui) fn detect(path: &str, is_dir: bool) -> FileType {
    if is_dir {
        return FileType {
            mime: MIME_FOLDER,
...
pub(in crate::ui) fn icon(file_type: &FileType) -> gtk::Image {
    icon_for_name(file_type.icon_name)
}

pub(in crate::ui) fn icon_for_name(icon_name: &str) -> gtk::Image {
...
pub(in crate::ui) fn set_icon_for_name(image: &gtk::Image, icon_name: &str) {
    if let Some(icon_path) = bundled_icon_path(icon_name) {
        let file = gio::File::for_path(icon_path);
        let paintable = gtk::IconPaintable::for_file(&file, 16, 1);
...
pub(crate) fn preview_kind_for_path(path: &str, is_dir: bool) -> PreviewKind {
...
pub(crate) fn is_sqlite_database_name(path: &str) -> bool {
    let file_name = base_name(path).to_ascii_lowercase();
    Path::new(&file_name)
        .extension()
        .and_then(|extension| extension.to_str())
...
fn icon_name_from_name_extension(file_name: &str, extension: &str) -> &'static str {
    ...
    match extension {
        ...
        | "png" | "tif" | "tiff" | "webp" => "image-x-generic-symbolic",
