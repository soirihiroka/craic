...
pub(crate) const INDIVIDUAL_CONTAINERS_GROUP: &str = "Individual Containers";
pub(crate) const COMPOSE_PROJECT_LABEL: &str = "com.docker.compose.project";
pub(crate) const COMPOSE_WORKING_DIR_LABEL: &str = "com.docker.compose.project.working_dir";
pub(crate) const COMPOSE_CONFIG_FILES_LABEL: &str = "com.docker.compose.project.config_files";
pub(crate) const COMPOSE_ENVIRONMENT_FILE_LABEL: &str =
...
impl ContainerInventory {
    ...
    pub(crate) fn group_by_key(&self, key: &str) -> Option<&ContainerGroup> {
        self.groups.iter().find(|group| group.key == key)
    }

    pub(crate) fn container_by_id(&self, id: &str) -> Option<&ContainerSummary> {
        self.groups
            .iter()
...
impl ContainerGroup {
    ...
    pub(crate) fn compose_metadata(&self) -> Option<&ComposeProject> {
        self.compose.as_ref()
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum ContainerGroupKind {
...
pub(crate) type DockerContainer = ContainerSummary;
pub(crate) type ContainerState = String;
...
impl ContainerSummary {
    ...
    pub(crate) fn action_enablement(&self) -> ContainerActionEnablement {
...
pub(crate) struct ContainerActionEnablement {
    pub start: bool,
    pub stop: bool,
    pub restart: bool,
    pub remove: bool,
...
impl ContainerActionEnablement {
    pub(crate) fn is_enabled(self, action: ContainerAction) -> bool {
        match action {
...
pub(crate) fn list_inventory(access: &dyn DockerAccess) -> Result<ContainerInventory, String> {
    list_containers(access).map(|containers| ContainerInventory {
        groups: group_containers(containers),
    })
}

pub(crate) fn list_containers(access: &dyn DockerAccess) -> Result<Vec<ContainerSummary>, String> {
...
pub(crate) fn group_containers(containers: Vec<ContainerSummary>) -> Vec<ContainerGroup> {
...
pub(crate) fn run_container_action(
    access: &dyn DockerAccess,
    container: &str,
...
pub(crate) fn run_compose_action(
    access: &dyn DockerAccess,
    compose: &ComposeProject,
...
pub(crate) fn state_is_running(state: impl AsRef<str>) -> bool {
    matches!(
...
pub(crate) fn can_start(state: impl AsRef<str>) -> bool {
    matches!(
        normalized_state(state.as_ref()).as_str(),
        "created" | "exited"
    )
...
pub(crate) fn can_stop(state: impl AsRef<str>) -> bool {
    matches!(
        normalized_state(state.as_ref()).as_str(),
        "running" | "restarting"
    )
...
pub(crate) fn can_restart(state: impl AsRef<str>) -> bool {
    normalized_state(state.as_ref()) == "running"
}

pub(crate) fn can_remove(state: impl AsRef<str>) -> bool {
    matches!(
        normalized_state(state.as_ref()).as_str(),
        "created" | "exited" | "dead"
    )
...
pub(crate) fn compose_args(compose: &ComposeProject, args: &[&str]) -> Vec<String> {
