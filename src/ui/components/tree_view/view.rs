...
pub(in crate::ui) struct TreeRenderer<K, S> {
...
impl<K, S> TreeRenderer<K, S> {
    pub(in crate::ui) fn new<M, U>(mount: M, update: U) -> Self
    where
...
pub(in crate::ui) struct TreeView<K, S> {
    pub(in crate::ui) root: gtk::Overlay,
    pub(in crate::ui) list: gtk::Box,
    ...
    row_reconciler: RefCell<craic_diff_ui::gtk::BoxReconciler<K, TreeRenderState<K, S>>>,
    sticky_reconciler: RefCell<craic_diff_ui::gtk::FixedReconciler<K, TreeRenderState<K, S>>>,
...
{
    pub(in crate::ui) fn builder() -> TreeViewBuilder {
        TreeViewBuilder::new()
    }
}

pub(in crate::ui) struct TreeViewBuilder {
...
impl TreeViewBuilder {
    ...
    pub(in crate::ui) fn vscrollbar_policy(mut self, policy: gtk::PolicyType) -> Self {
        self.vscrollbar_policy = policy;
        self
    }

    pub(in crate::ui) fn autoscroll_context(mut self, context: &'static str) -> Self {
        self.autoscroll_context = context;
        self
    }

    pub(in crate::ui) fn canvas_scrollbar(mut self, enabled: bool) -> Self {
        self.canvas_scrollbar = enabled;
        self
    }

    pub(in crate::ui) fn build<K, S>(self) -> Rc<TreeView<K, S>>
    where
        K: Clone + Eq + Hash + std::fmt::Debug + 'static,
        S: Clone + PartialEq + 'static,
    {
        ...
        let tree = Rc::new(TreeView {
            ...
            row_reconciler: RefCell::new(craic_diff_ui::gtk::BoxReconciler::new()),
            sticky_reconciler: RefCell::new(craic_diff_ui::gtk::FixedReconciler::new()),
...
where
    ...
    S: Clone + PartialEq + 'static,
{
    pub(in crate::ui) fn set_rows(
        self: &Rc<Self>,
        rows: Vec<TreeRow<K, S>>,
        renderer: TreeRenderer<K, S>,
    ) -> craic_diff_ui::ReconcileStats {
    ...
    pub(in crate::ui) fn update_sticky_rows(&self) {
    ...
    pub(in crate::ui) fn sticky_row_at_viewport_y(&self, y: f64) -> Option<TreeRow<K, S>> {
        let scroll_y = self.scroller.vadjustment().value().max(0.0);
    ...
    pub(in crate::ui) fn last_sticky_row(&self) -> Option<TreeRow<K, S>> {
        let scroll_y = self.scroller.vadjustment().value().max(0.0);
        sticky_items(&self.rows.borrow(), scroll_y)
            .last()
    ...
    pub(in crate::ui) fn row_at_content_y(&self, y: f64) -> Option<TreeRow<K, S>> {
        if y < 0.0 {
            return None;
        }
    ...
    pub(in crate::ui) fn last_row_before_content_y_matching<F>(
        &self,
        y: f64,
        mut matches: F,
    ) -> Option<TreeRow<K, S>>
    ...
    pub(in crate::ui) fn scroll_row_into_view(&self, key: &K) {
    ...
    pub(in crate::ui) fn focus_row(&self, key: &K) -> bool {
    ...
    pub(in crate::ui) fn has_row_focus(&self) -> bool {
    ...
    pub(in crate::ui) fn has_edit_focus(&self) -> bool {
        let mut child = self.list.first_child();
        while let Some(widget) = child {
    ...
    pub(in crate::ui) fn connect_pointer_press<F>(self: &Rc<Self>, callback: F)
    where
    ...
    pub(in crate::ui) fn scroll_row_below_sticky(&self, key: &K) {
    ...
    pub(in crate::ui) fn row_is_below_sticky(&self, key: &K) -> bool {
        let Some((row_top, _, top_inset)) = self.row_scroll_geometry(key) else {
            return false;
        };
    ...
    pub(in crate::ui) fn draw_disclosure(
        &self,
        key: &K,
        area: &gtk::DrawingArea,
    ...
    pub(in crate::ui) fn prepare_disclosure(&self, key: &K, expanded: bool) -> bool {
        self.disclosure.prepare(key, expanded)
    }

    pub(in crate::ui) fn animate_disclosure(self: &Rc<Self>, area: &gtk::DrawingArea, key: K) {
        let tree = self.clone();
    ...
    pub(in crate::ui) fn disclosure_widget(
        self: &Rc<Self>,
        key: K,
        expanded: bool,
    ) -> gtk::DrawingArea {
