...
impl WorkspaceKey {
    ...
    pub(crate) fn git_remote_url(&self) -> Option<&str> {
...
pub(crate) struct AgentSessionRow {
    ...
    pub(crate) normalized_task_description: Option<String>,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct WorkspaceTag {
    pub(crate) tag: String,
    pub(crate) session_count: usize,
}

#[derive(Clone, Debug)]
pub(crate) struct AgentSessionUpsert {
    pub(crate) provider_id: String,
    pub(crate) workspace: WorkspaceKey,
    pub(crate) title: String,
    pub(crate) initial_restore_state: RestoreState,
    pub(crate) session_uuid: Option<String>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum CodexMappingOutcome {
    Restorable(String),
    Ambiguous,
    Missing,
    Unsupported,
...
pub(crate) fn workspace_for_system_path(
...
pub(crate) fn default_title_should_persist(title: &str) -> bool {
...
pub(crate) fn upsert_session(input: AgentSessionUpsert) -> Result<AgentSessionRow, String> {
...
pub(crate) fn upsert_session_for_manual_id(
...
pub(crate) fn lookup_session(local_id: i64) -> Result<Option<AgentSessionRow>, String> {
    let conn = history_connection().map_err(db_error)?;
    conn.query_row(
...
pub(crate) fn update_session_summary(
    local_id: i64,
...
pub(crate) fn workspace_tags(workspace_key: &str) -> Result<Vec<String>, String> {
    let conn = history_connection().map_err(db_error)?;
    let mut stmt = conn
        .prepare(
            "SELECT tag.tag
...
pub(crate) fn workspace_tag_counts(workspace_key: &str) -> Result<Vec<WorkspaceTag>, String> {
    let conn = history_connection().map_err(db_error)?;
    let mut stmt = conn
        .prepare(
...
pub(crate) fn max_local_session_id() -> Result<Option<i64>, String> {
    let conn = history_connection().map_err(db_error)?;
...
pub(crate) fn update_session_title(local_id: i64, title: &str) -> Result<AgentSessionRow, String> {
    let title = normalize_title(title);
    if !default_title_should_persist(&title) {
        return Err("default agent session title is not persisted".to_string());
    }
...
pub(crate) fn set_manual_session_id(local_id: i64, cli_session_id: &str) -> Result<(), String> {
...
pub(crate) fn suggested_cli_session_id(row: &AgentSessionRow) -> Option<String> {
    row.cli_session_id
        .clone()
        .or_else(|| metadata_string(&row.metadata_json, "candidate_cli_session_id"))
}

pub(crate) fn cli_session_id_is_empty(local_id: i64) -> Result<bool, String> {
...
pub(crate) fn mark_ended(local_id: i64) -> Result<(), String> {
    let conn = history_connection().map_err(db_error)?;
    let now = unix_now_ms();
    conn.execute(
        "UPDATE agent_sessions
...
pub(crate) fn delete_session(local_id: i64) -> Result<(), String> {
    let conn = history_connection().map_err(db_error)?;
    let deleted = conn
        .execute(
...
pub(crate) fn map_codex_session(local_id: i64) -> Result<CodexMappingOutcome, String> {
    let Some(row) = lookup_session(local_id)? else {
