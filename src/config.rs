...
pub fn load() -> AppConfig {
    ...
    let commit_message_provider = normalized_config_string(
        config_string(&config, COMMIT_MESSAGE_PROVIDER_KEY)
            .unwrap_or_else(|| crate::agent_provider::DEFAULT_PROVIDER_ID.to_string()),
    )
    .unwrap_or_else(|| crate::agent_provider::DEFAULT_PROVIDER_ID.to_string());
...
pub(crate) fn workspace_color_for(
    provider_id: &str,
    workspace_root: &str,
) -> Option<WorkspaceColor> {
    let config = load();
...
pub(crate) fn smart_feature_shell_providers() -> &'static [&'static str] {
    &["codex", "agy", "opencode"]
}
...
fn default_config() -> ConfigMap {
    HashMap::from([
        ...
        (
            COMMIT_MESSAGE_PROVIDER_KEY.to_string(),
            toml::Value::String(crate::agent_provider::DEFAULT_PROVIDER_ID.to_string()),
        ),
        (
            SHELL_FONT_SIZE_KEY.to_string(),
...
pub(crate) fn sessions_db_path() -> Option<PathBuf> {
...
pub(crate) fn prompt_dirs(repo_path: Option<&Path>) -> Vec<PathBuf> {
...
pub(crate) fn expand_config_path_for_ui(path: &str) -> Option<PathBuf> {
    expand_home(path)
}

pub(crate) fn craic_dir() -> Option<PathBuf> {
