...
use crate::system::capabilities::{
    open::OpenAccess, shell::ShellAccess, terminal_link::TerminalLinkAccess,
};
...
const JETBRAINS_MONO_MARKER: &str = "JetBrainsMonoNerdFontMono-Regular.ttf";
const GIT_SNAPSHOT_POLL_INTERVAL: Duration = Duration::from_millis(75);
pub(in crate::ui) const AGENT_SESSION_NOTIFICATION_ACTION: &str = "open-agent-session";
pub(in crate::ui) const AGENT_SESSION_NOTIFICATION_DETAILED_ACTION: &str = "app.open-agent-session";

pub(in crate::ui) fn agent_session_notification_id(session_id: u64) -> String {
...
pub fn build_ui(app: &adw::Application, launch_start: Instant) {
    ...
    let page_context = pages::PageContext::new(
        ...
        Rc::new({
            ...
            move |command, title| {
                if let Some(state) = state_slot.borrow().upgrade() {
                    state.content.run_terminal_command(command, title)
                } else {
                    Err("Application is not ready.".to_string())
                }
            }
...
pub(crate) fn asset_search_paths() -> Vec<PathBuf> {
    ...
    if let Ok(current_dir) = std::env::current_dir() {
        paths.push(current_dir.join("src").join("assets"));
    }

    if let Ok(exe_path) = std::env::current_exe() {
        let mut current = exe_path.parent();
        while let Some(dir) = current {
            let candidate = dir.join("src").join("assets");
            if candidate.join("text-rust-symbolic.svg").is_file() {
    ...
    if let Ok(data_home) = std::env::var("XDG_DATA_HOME") {
        paths.push(PathBuf::from(data_home).join("craic").join("assets"));
    }

    if let Ok(home) = std::env::var("HOME") {
        paths.push(home_data_path(&home));
    }

    let data_dirs = std::env::var("XDG_DATA_DIRS")
        .unwrap_or_else(|_| "/usr/local/share:/usr/share".to_string());
    for data_dir in data_dirs.split(':').filter(|path| !path.is_empty()) {
        paths.push(PathBuf::from(data_dir).join("craic").join("assets"));
    }

    paths
}

fn home_data_path(home: &str) -> PathBuf {
    PathBuf::from(home)
        .join(".local")
        .join("share")
        .join("craic")
...
fn font_search_paths() -> Vec<PathBuf> {
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        let mut current = exe_path.parent();
        while let Some(dir) = current {
            let candidate = dir.join("src").join("fonts").join(JETBRAINS_MONO_DIR);
            if candidate.join(JETBRAINS_MONO_MARKER).is_file() {
                paths.push(candidate);
                break;
            }
            current = dir.parent();
...
fn refresh_active_repo_metadata(state: &Rc<AppState>, item_id: Option<String>) {
    ...
    let github_access = state.providers.github(&system.id, &workspace);
    log::debug!(
