...
impl SyntaxHighlighter {
    pub(crate) fn new(language_name: &str) -> Self {
    ...
    pub(crate) fn set_language(&mut self, language_name: &str) {
    ...
    pub(crate) fn apply_edit(&mut self, start: usize, old_end: usize, replacement: &str) {
    ...
    pub(crate) fn highlight_current(&self) -> Vec<HighlightRange> {
    ...
    pub(crate) fn fold_ranges_current(&self) -> Vec<(usize, usize)> {
        let Some(tree) = self.tree.as_ref() else {
            return Vec::new();
        };
    ...
    pub(crate) fn has_error_current(&self) -> bool {
        self.tree
            .as_ref()
            .is_some_and(|tree| tree.root_node().has_error())
    }

    pub(crate) fn syntax_issues_current(&self) -> Vec<SyntaxIssue> {
        let Some(tree) = self.tree.as_ref() else {
            return Vec::new();
        };
    ...
    pub(crate) fn completions_current(&self, cursor: usize) -> Option<suggest::CompletionSet> {
...
pub(crate) fn language_hint_from_path(path: &str) -> String {
...
pub(crate) fn apply_edit_to_ranges(
    ranges: &mut Vec<HighlightRange>,
    start: usize,
    old_end: usize,
