...
pub struct GitConfig {
    ...
    pub use_system_timezone: Option<bool>,
    pub warn_if_remote_owner_mismatch: Option<bool>,
    pub github_auth_account: Option<GitHubAuthAccount>,
}
...
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
pub struct QuickActionAdditionalConfig {
    #[serde(default)]
...
pub fn git_config(path: &Path) -> GitConfig {
...
pub fn git_config_from_file_access(files: &dyn FileAccess) -> GitConfig {
...
pub fn save_git_config_with_file_access(
    files: &dyn FileAccess,
    commit_timezone: &str,
    warn_if_remote_owner_mismatch: bool,
    use_system_timezone: bool,
    github_auth_account: Option<&GitHubAuthAccount>,
) -> Result<(), String> {
...
pub fn quick_action_additional_commands(path: &Path) -> Vec<QuickActionAdditionalConfig> {
    load(path)
        .quick_action
...
pub fn markdown_lint_ignored_rules_from_file_access(files: &dyn FileAccess) -> Vec<String> {
...
pub fn add_markdown_lint_ignore_with_file_access(
    files: &dyn FileAccess,
    rule_name: &str,
...
pub fn commit_convention_from_file_access(files: &dyn FileAccess) -> Option<String> {
...
pub fn normalize_timezone(value: &str) -> Result<String, String> {
...
fn local_github_auth_account(config: &LocalGitHubConfig) -> Option<GitHubAuthAccount> {
    ...
    Some(GitHubAuthAccount {
        host: host.to_string(),
        login: login.to_string(),
    })
}

fn local_github_config(account: Option<&GitHubAuthAccount>) -> LocalGitHubConfig {
