...
#[derive(Clone, PartialEq)]
pub struct TreeRenderState<K, S> {
    pub row: TreeRow<K, S>,
    pub sticky: bool,
    pub bottom: bool,
    pub y: f64,
...
#[derive(Clone)]
pub struct StickyLayoutItem<K, S> {
    pub row: TreeRow<K, S>,
    pub draw_y: f64,
...
pub fn sticky_items<K, S>(rows: &[TreeRow<K, S>], scroll_y: f64) -> Vec<StickyLayoutItem<K, S>>
where
    K: Clone,
    S: Clone,
{
