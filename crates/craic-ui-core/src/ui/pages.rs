...
pub type PageRef = Rc<dyn Page + 'static>;
pub type PageInitializeComplete = Box<dyn FnOnce(gtk::Widget, gtk::Widget) + 'static>;
pub type PageRefreshComplete = Rc<dyn Fn() + 'static>;
pub type GitSnapshotComplete =
    Box<dyn FnMut(String, Result<crate::git::RepositorySnapshot, String>) + 'static>;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PageRefreshRequest {
    WorkspaceSnapshot,
    Custom,
}

#[derive(Clone)]
pub struct PageBadge {
    text: String,
}
...
impl PageContext {
    ...
    pub fn local_workspace_path(&self) -> Option<PathBuf> {
    ...
    pub fn git_unavailable_message(&self) -> String {
        format!(
    ...
    pub fn shell(&self) -> Option<Arc<dyn ShellAccess>> {
        self.providers
            .shell(&self.system_ref.borrow().id, &self.workspace_ref())
    }

    pub fn docker(&self) -> Option<Arc<dyn DockerAccess>> {
        self.providers
            .docker(&self.system_ref.borrow().id, &self.workspace_ref())
    }

    pub fn desktop_opener(&self) -> Option<Arc<dyn DesktopOpenAccess>> {
        self.providers
    ...
    pub fn desktop_opener_unavailable_message(&self) -> String {
        format!(
    ...
    pub fn url_opener_unavailable_message(&self) -> String {
        format!(
    ...
    pub fn show_error(&self, heading: &str, message: &str) {
        if let Some(window) = self.window() {
            show_error_dialog(&window, heading, message);
        }
    }

    pub fn refresh(&self, message: Option<String>) {
        (self.refresh)(message, true);
    }

    pub fn refresh_without_toast(&self, message: Option<String>) {
    ...
    pub fn git_action_running(&self) -> bool {
        self.git_action_running.get()
    }

    pub fn run_git_action(&self) {
        (self.run_git_action)();
    }
    ...
    pub fn request_git_snapshot<F>(
        &self,
        workspace_key: String,
        git_handle: Arc<crate::git::GitRepoHandle>,
        on_result: F,
    ) where
        F: FnMut(String, Result<crate::git::RepositorySnapshot, String>) + 'static,
    {
        (self.request_git_snapshot)(workspace_key, git_handle, Box::new(on_result));
    }

    pub fn notify_badge_changed(&self) {
        (self.notify_badge_changed)();
    }

    pub fn dispatch_command(&self, command: PageCommand) {
...
impl PageHost {
    pub fn new(left_slot: &gtk::Box, right_slot: &gtk::Box) -> Self {
    ...
    pub fn left_hovered(&self) -> bool {
        self.left_hovered.get()
    }

    pub fn right_hovered(&self) -> bool {
        self.right_hovered.get()
    }

    pub fn show(&self, pages: &[PageRef], index: usize) {
