...
pub fn word_bounds_at(text: &str, offset: usize) -> Option<(usize, usize)> {
    ...
    let (_, character) = next_char(text, offset)?;
    let group = selectable_group(character)?;
    let mut start = offset;
    while let Some((previous, character)) = previous_char(text, start) {
        if selectable_group(character) != Some(group) {
            break;
        }
        start = previous;
    }
    let mut end = offset;
    while let Some((current, character)) = next_char(text, end) {
        if selectable_group(character) != Some(group) {
            break;
        }
        end = current + character.len_utf8();
    }
    Some((start, end))
}
    ...
    LineBounds: Fn(T) -> Option<(T, T)>,
{
    match mode {
        ...
        SelectionMode::Word => word_bounds(point).map_or_else(
            || {
                let selection = AnchoredSelection {
                    anchor: point,
                    focus: point,
        ...
        SelectionMode::Line => line_bounds(point).map_or_else(
            || {
                let selection = AnchoredSelection {
                    anchor: point,
                    focus: point,
...
fn selectable_group(character: char) -> Option<TextGroup> {
    (!character.is_whitespace()).then(|| text_group(character))
}

fn text_group(character: char) -> TextGroup {
