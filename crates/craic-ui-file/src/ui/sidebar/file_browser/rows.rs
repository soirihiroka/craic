...
pub struct NewEntryRow {
    pub folder: FileNodePath,
    pub default_name: String,
    pub depth: usize,
    pub kind: super::NewEntryKind,
}

#[derive(Clone, PartialEq, Eq)]
pub struct RenameEntryRow {
    pub row: BrowserRow,
...
impl FileBrowser {
    pub fn connect_browser_rows(self: &Rc<Self>) {
    ...
    pub fn set_browser_rows(self: &Rc<Self>, mut rows: Vec<BrowserListRow>) {
    ...
    pub fn refresh_browser_row_state(self: &Rc<Self>) {
    ...
    pub fn install_folder_drop_targets<W: IsA<gtk::Widget>>(
        self: &Rc<Self>,
        widget: &W,
        folder: FileNodePath,
    ) {
    ...
    pub fn browser_list_row_at_y(&self, y: f64) -> Option<BrowserListRow> {
        self.tree
            .row_at_content_y(y)
            .map(|row| row.state.to_row())
            .filter(|row| !matches!(row, BrowserListRow::RootGap))
    ...
    pub fn folder_for_gap_at_y(&self, y: f64) -> Option<FileNodePath> {
        if matches!(
...
pub enum BrowserListRowRenderState {
    Tree {
        row: BrowserRow,
        expanded: bool,
        selected: bool,
...
impl BrowserListRowRenderState {
    pub fn to_row(&self) -> BrowserListRow {
        match self {
...
pub fn browser_list_row_key(row: &BrowserListRow, index: usize) -> BrowserListRowKey {
    match row {
...
pub fn tree_row_key(row: &BrowserRow) -> BrowserListRowKey {
