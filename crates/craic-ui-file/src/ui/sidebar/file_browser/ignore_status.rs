...
impl FileBrowser {
    pub fn refresh_git_ignore_cache_for_rules(&self, rows: &[BrowserRow]) {
        if rows
            .iter()
    ...
    pub fn apply_git_ignore_cache(&self, rows: &mut [BrowserRow]) {
    ...
    pub fn queue_git_ignore_query(self: &Rc<Self>, rows: &[BrowserRow]) {
        if rows.is_empty() {
            return;
        }
    ...
    pub fn clear_git_ignore_cache(&self) {
        self.git_ignore_cache.borrow_mut().clear();
        self.git_ignore_generation
            .set(self.git_ignore_generation.get().wrapping_add(1));
    }
...
pub struct GitIgnoreCacheEntry {
...
#[derive(PartialEq, Eq)]
pub struct GitIgnoreRuleFileSignature {
    path: FileNodePath,
