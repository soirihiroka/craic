...
pub use self::file_ops::NewEntryKind;
...
impl FileBrowser {
    ...
    pub fn root_node_path(&self) -> FileNodePath {
        self.file_access.borrow().root()
    }

    pub fn node_path(&self, relative: &str) -> FileNodePath {
        node_path_for_relative(&self.root_node_path(), relative)
    }

    pub fn set_desktop_opener(&self, desktop_opener: Option<Arc<dyn DesktopOpenAccess>>) {
        self.desktop_opener.replace(desktop_opener);
    }

    pub fn set_terminal_actions_available(&self, available: bool) {
        self.terminal_actions_available.set(available);
    }

    pub fn set_container_actions_available(&self, available: bool) {
        self.container_actions_available.set(available);
    }

    pub fn selected_node_path(&self) -> FileNodePath {
    ...
    pub fn reveal_workspace_path(self: &Rc<Self>, path: &str) {
        if !self.search_query.borrow().is_empty() {
    ...
    pub fn connect_search_match_selected<F>(&self, callback: F)
    where
    ...
    pub fn connect_open_terminal_requested<F>(&self, callback: F)
    where
    ...
    pub fn connect_run_in_terminal_requested<F>(&self, callback: F)
    where
        F: Fn(String) + 'static,
    {
    ...
    pub fn connect_add_to_chat_requested<F>(&self, callback: F)
    where
        F: Fn(String) + 'static,
    {
    ...
    pub fn connect_ignore_requested<F>(&self, callback: F)
    where
        F: Fn(String) + 'static,
    {
    ...
    pub fn connect_container_file_action_requested<F>(&self, callback: F)
    where
    ...
    pub fn connect_open_failed<F>(&self, callback: F)
    where
        F: Fn(String) + 'static,
    {
    ...
    pub fn toggle_dir(self: &Rc<Self>, path: &FileNodePath) {
        if !self.search_query.borrow().is_empty() {
            {
    ...
    pub fn scroll_selected_row_into_view(&self) {
    ...
    pub fn focus_browser_shell(&self) {
    ...
    pub fn focus_pending_new_entry(&self) {
    ...
    pub fn focus_pending_rename_entry(&self) {
    ...
    pub fn rebuild_if_changed(self: &Rc<Self>) {
        let rows = self.visible_rows();
        let status_signatures = self.changed_file_statuses.borrow();
        let signature = rows_signature(&rows, &status_signatures);
...
pub fn skipped_names() -> [&'static str; 1] {
    [".git"]
}
...
pub fn set_scroll_value(adjustment: &gtk::Adjustment, value: f64) {
...
pub fn node_path_for_relative(root: &FileNodePath, relative: &str) -> FileNodePath {
