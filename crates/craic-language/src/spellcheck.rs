...
pub struct SpellcheckAllowlist {
...
pub fn check_filename(name: &str, allowlist: &SpellcheckAllowlist) -> Vec<SpellcheckIssue> {
    let checker = Spellchecker::new(allowlist);
    let mut issues = Vec::new();
...
pub fn manifest_allowlist_from_texts(manifests: &[(&str, &str)]) -> SpellcheckAllowlist {
...
impl SpellcheckAllowlist {
    pub fn insert_name(&mut self, value: &str) {
