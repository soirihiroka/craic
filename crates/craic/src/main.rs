...
mod linux {
    ...
    fn startup_workspace_arg() -> StartupWorkspaceArg {
        ...
        if args.len() > 1 {
            ...
            {
                return startup_navigation_args(&args).unwrap_or_else(|error| {
                    StartupWorkspaceArg {
                        workspace: None,
                        open_location: None,
                        error: Some(error),
    ...
    fn startup_navigation_args(args: &[OsString]) -> Result<StartupWorkspaceArg, String> {
        ...
        Ok(StartupWorkspaceArg {
            ...
            open_location: open_path.map(|path| craic_ui::StartupOpenLocation {
                path,
                line,
                column,
            }),
...
fn main() {
    eprintln!("Craic does not have a frontend for this platform.");
}
