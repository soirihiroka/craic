...
def git_bytes(repo, args, success=(0,)):
    ...
    if output.returncode not in success:
...
def git_text(repo, args, success=(0,)):
    return git_bytes(repo, args, success).decode("utf-8", "replace").strip()
...
def workdir_bytes(repo, file_path, max_bytes):
    ...
    if size > max_bytes:
        fail(f"{pathlib.PurePosixPath(file_path).name} is too large to preview as text.")
...
def worktree_diff(repo, pair, fallback_path):
    ...
    if old_path is None and new_path:
        return git_bytes(
            ...
            [
                ...
                "--",
                "/dev/null",
                str(pathlib.Path(repo) / new_path),
            ],
            success=(0, 1),
        )
    return git_bytes(
        ...
        [
            ...
            "--unified=3",
            "--",
            *unique_paths(old_path, new_path, fallback_path),
        ],
    )


def commit_diff(repo, commit_hash, pair, fallback_path):
    return git_bytes(
        repo,
        [
            "show",
            ...
            commit_hash,
            "--",
            *unique_paths(pair.get("old"), pair.get("new"), fallback_path),
        ],
    )
...
max_text_bytes = int(request["max_text_bytes"])

if mode == "worktree":
    pair = worktree_path_pair(repo, file_path)
    old_path = pair.get("old") or file_path
    new_path = pair.get("new") or file_path
    left = tree_bytes(repo, "HEAD", old_path, max_text_bytes)
    right = workdir_bytes(repo, new_path, max_text_bytes)
    diff = worktree_diff(repo, pair, file_path)
elif mode == "commit":
    ...
    left = tree_bytes(repo, parent, old_path, max_text_bytes)
    right = tree_bytes(repo, commit_hash, new_path, max_text_bytes)
    diff = commit_diff(repo, commit_hash, pair, file_path)
else:
    fail(f"Unsupported diff mode: {mode}")
...
json.dump(
    {
        ...
        "left_b64": b64(left),
        "right_b64": b64(right),
        "paths_changed": bool(pair.get("old") and pair.get("new") and pair.get("old") != pair.get("new")),
    },
    sys.stdout,
)
