...
pub fn parse_porcelain_status_entries(bytes: &[u8]) -> Vec<GitStatusEntry> {
    let tokens = bytes
        .split(|byte| *byte == 0)
...
pub fn status_entry_visible(entry: &GitStatusEntry) -> bool {
    entry.status_code != "AD"
}

pub fn changed_file_from_porcelain_entry(entry: &GitStatusEntry) -> ChangedFile {
    ChangedFile {
...
pub fn porcelain_entry_matches_path(entry: &GitStatusEntry, path: &str) -> bool {
    entry.path == path || entry.old_path.as_deref() == Some(path)
}

pub fn porcelain_entry_force_remove_paths(entry: &GitStatusEntry) -> Vec<String> {
...
pub fn porcelain_entry_update_paths(entry: &GitStatusEntry) -> Vec<String> {
...
impl FileComparison {
    pub fn from_rows(rows: Vec<FileDiffRow>) -> Self {
...
impl BytesComparison {
    pub fn from_parts(before: Option<Vec<u8>>, after: Option<Vec<u8>>) -> Self {
