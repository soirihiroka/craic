                ...
                match receiver.try_recv() {
                    ...
                    Ok(GitCommandEvent::Failed { message: err }) => {
                        ...
                        if matches!(action, GitAction::Pull | GitAction::PullPush)
                            && craic_vcs::git::is_local_changes_overwritten_error(&err)
                        {
                            state.content.update(&snapshot, false);
                            let files = craic_vcs::git::parse_files_to_be_overwritten(&err);
                            log::info!(
...
fn local_changes_overwritten_body(action: &GitAction, files: &[String]) -> String {
    craic_vcs::git::local_changes_overwritten_body(files, matches!(action, GitAction::PullPush))
}
