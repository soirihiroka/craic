...
impl CodeEditor {
    ...
    pub fn connect_scroll_changed<F>(&self, callback: F)
    where
        F: Fn(f64) + 'static,
    {
        self.state
    ...
    pub fn source_offset_at_scroll_top(&self) -> usize {
        render::source_offset_at_scroll_top(&self.area, &self.state)
    }

    pub fn set_source_offset_at_scroll_top(&self, offset: usize) {
    ...
    pub fn connect_markdown_lint_ignore<F>(&self, callback: F)
    where
        F: Fn(String) + 'static,
    {
        self.state
