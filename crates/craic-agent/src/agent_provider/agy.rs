...
impl AgentProvider for Provider {
    ...
    fn check_available(&self) -> Result<(), String> {
        check_command_available(self.label(), &command_path("agy"))
    }

    fn fetch_models(&self) -> Result<Vec<ModelOption>, String> {
        let output = run_model_command(self, command_path("agy"), &["models"])?;
    ...
    fn generate_text(
        ...
        run_generation_command(
            self,
            CommandInvocation::new(command_path("agy"), args),
            prompt,
            cancellation,
        )
    }
